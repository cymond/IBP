...
'''
After updates Report on...
1. Data Timeliness/
    * Offset from market with latest data
2. Manage rolls
    * Pending roll!
    * Days to roll
    * Missing Roll dates
3. History repricing
    * Give warning
4. Quandl IB mismatch
5. price stddev.
    * Unexpected Variation

6. Create Shadow file for each intrument which will have
    DATE
    PRICE
    CARRY
    PRICE_VOUME
...
def get_new_data(market, source, file_name, date):
    if os.path.isfile(file_name):
        df = pd.read_csv(file_name, usecols=[0, 1], names=['DATETIME', ['PRICE']])
        df.set_index(['DATETIME'], inplace=True)

        if not df.empy:
            last_day = df.ilock[-1].DATETIME

        df = df.loc[date:][1:]
        new_updates = False
        if df.empty:
            logger.debug("NO NU DATA: {:<10}: {} last: {}".format(market, source, last_day))
        else:
            logger.info("NEW DATA  : {:<10}: {} last: {}".format(market, source, last_day))
            new_updates = True

    return [new_updates, df]


        quandl_price_updates = True  # set update file to TRUE
...
def update_futures_market(rowdf, new_data_path, legacy_data_path):
    ...
    return_list = get_new_data(market, 'Quandl ' , quandl_price_file, last_price_date)
    quandl_price_updates = return_list[0]
    quandl_df = return_list[1]
    '''
    if os.path.isfile(quandl_price_file):                                # Check that download file exists
        price_file = True
        quandl_price_df = pd.read_csv(quandl_price_file, usecols=[0, 1]) # Read it
        quandl_price_df.columns = ['DATETIME', 'PRICE']
        if not quandl_price_df.empty:                                    # Check whether it's empty
            last_quandl_date = quandl_price_df.iloc[-1].DATETIME         # DATETIME value of last record
        quandl_df = quandl_price_df.set_index('DATETIME').copy()         # index the DF
        quandl_df = quandl_df.loc[last_price_date:][1:]                  # select only newer record/s
        if quandl_df.empty:
                   ...
                   if days_to_roll.days < 0:
                       ...
                       if ndays < 6:
                           ...
                           logger.info("ROLL INFO : {:<10}: ({} to {}) Due in (days) : {:<3}".format(market.CARVER,
                                                                                                     ...
                                                                                                     next_price_mat,
                                                                                                     ndays))
                update_futures_market(market_df, source_path, destination_path)
            else:
                update_forex_market(market_df, source_path, destination_path)
